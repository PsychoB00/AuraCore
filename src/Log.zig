...
pub const log = struct {
    ...
    pub fn Logger(comptime Options: LoggerOptions, comptime LogType: type, comptime LogProcessorType: type) type {
        return struct {
            const LoggerType = Logger(Options, LogType, LogProcessorType);
            ...
            /// Initialize Logger
            ///
            /// Logger doesn't keep `timezone`, it's passed to LogProcessor
            pub fn init(timezone: ?*const zeit.TimeZone) LoggerType {
                return .{
                    .thread_running = atomic(bool).init(false),
                    .thread = null,
                    .uncollected = atomic(u32).init(0),
                    .log_pool = null,
                    .reserve_index = null,
                    .log_processor = LogProcessorType.init(timezone),
                };
            }
            ...
            pub fn spawn(self: *LoggerType) !void {
                ...
                self.thread = try std.Thread.spawn(
                    .{
                        .stack_size = (Options.log_pool_size * @sizeOf(LogType)) + @sizeOf(atomic(u32)),
                    },
                    _collect,
                    .{
                        self,
            ...
            fn _collect(self: *LoggerType) void {
                ...
                while (self.thread_running.load(.acquire) or self.uncollected.load(.acquire) != 0) {
                    ...
                    for (&self.log_pool.?) |*log_request| {
                        _ = @cmpxchgWeak(State, &log_request.state.raw, .constructed, .collected, .acq_rel, .acquire) orelse {
            ...
            pub fn log(self: *LoggerType, comptime LogLevel: Level) *LogType {
                ...
                while (true) {
                    ...
                    _ = @cmpxchgWeak(u32, &self.reserve_index.?.raw, Options.log_pool_size, 0, .acq_rel, .acquire);

                    _ = @cmpxchgWeak(State, &self.log_pool.?[self.reserve_index.?.load(.acquire)].state.raw, .empty, .reserved, .acq_rel, .acquire) orelse {
    ...
    pub fn LogFmt(comptime Options: LogFmtOptions, comptime LogType: type) type {
        return struct {
            ...
            pub fn levelFmt(arg: *const LogType, writer: std.io.AnyWriter) void {
                ...
                const delimiter_index = comptime std.mem.indexOfScalarPos(u8, Options.level_fmt, 0, '%') orelse
                ...
                if (comptime delimiter_index > 0)
                    writer.writeAll(Options.level_fmt[0..delimiter_index]) catch unreachable;

                switch (placeholder) {
                    inline 'l' => writer.writeAll(@tagName(arg.*.level.?)) catch unreachable,
                    inline 'L' => {
                        switch (arg.*.level.?) {
                            .debug => writer.writeAll("DEBUG") catch unreachable,
                            .info => writer.writeAll("INFO") catch unreachable,
                            .warn => writer.writeAll("WARN") catch unreachable,
                            .err => writer.writeAll("ERROR") catch unreachable,
                            .fatal => writer.writeAll("FATAL") catch unreachable,
                        }
                    },
                ...
                if (comptime delimiter_index + 2 < Options.level_fmt.len)
                    writer.writeAll(Options.level_fmt[(delimiter_index + 2)..]) catch unreachable;
            }

            /// Writes `Log.instant` converted to time based on `timezone`, to `writer` based on `Options.time_fmt`
            pub fn timeFmt(arg: *const LogType, writer: std.io.AnyWriter, timezone: *const zeit.TimeZone) void {
                assert(arg.*.instant != null);

                const time = arg.*.instant.?.in(timezone).time();
                time.strftime(writer, Options.time_fmt) catch unreachable;
            }

            /// Writes `Log.context` to `writer` based on `Options.scope_fmt`
            pub fn scopeFmt(arg: *const LogType, writer: std.io.AnyWriter) void {
                assert(arg.*.context != null);
                assert(arg.*.context_len != null);

                const delimiter_index = comptime std.mem.indexOfScalarPos(u8, Options.scope_fmt, 0, '%') orelse
                ...
                if (comptime delimiter_index > 0)
                    writer.writeAll(Options.scope_fmt[0..delimiter_index]) catch unreachable;

                if (comptime placeholder == 's')
                    writer.writeAll(arg.*.context.?[0..arg.*.context_len.?]) catch unreachable
                else
                    @compileError("`Options.scope_fmt` doesn't contain valid placeholder");

                if (comptime delimiter_index + 2 < Options.scope_fmt.len)
                    writer.writeAll(Options.scope_fmt[(delimiter_index + 2)..]) catch unreachable;
            }

            /// Writes `Log.message` to `writer` based on `Options.message_fmt`
            pub fn messageFmt(arg: *const LogType, writer: std.io.AnyWriter) void {
                assert(arg.*.message != null);
                assert(arg.*.message_len != null);

                const delimiter_index = comptime std.mem.indexOfScalarPos(u8, Options.message_fmt, 0, '%') orelse
                ...
                if (comptime delimiter_index > 0)
                    writer.writeAll(Options.message_fmt[0..delimiter_index]) catch unreachable;

                if (comptime placeholder == 'm')
                    writer.writeAll(arg.*.message.?[0..arg.*.message_len.?]) catch unreachable
                else
                    @compileError("`Options.message_fmt` doesn't contain valid placeholder");

                if (comptime delimiter_index + 2 < Options.message_fmt.len)
                    writer.writeAll(Options.message_fmt[(delimiter_index + 2)..]) catch unreachable;
            }

            /// Writes `Log.source_location` to `writer` based on `Options.source_location_fmt`
            pub fn sourceLocationFmt(arg: *const LogType, writer: std.io.AnyWriter) void {
                assert(arg.*.source_location != null);
                comptime var origin_point: usize = 0;

                inline while (true) {
                    const delimiter_index: usize = comptime std.mem.indexOfScalarPos(u8, Options.source_location_fmt, origin_point, '%') orelse
                        break;
                    ...
                    if (comptime delimiter_index > origin_point)
                        writer.writeAll(Options.source_location_fmt[origin_point..delimiter_index]) catch unreachable;

                    switch (placeholder) {
                        inline 'm' => writer.writeAll(arg.*.source_location.?.module) catch unreachable,
                        inline 'F' => writer.writeAll(arg.*.source_location.?.file) catch unreachable,
                        inline 'f' => writer.writeAll(arg.*.source_location.?.fn_name) catch unreachable,
                        inline 'l' => writer.print("{}", .{arg.*.source_location.?.line}) catch unreachable,
                        inline 'c' => writer.print("{}", .{arg.*.source_location.?.column}) catch unreachable,
                ...
                if (comptime origin_point < Options.source_location_fmt.len)
                    writer.writeAll(Options.source_location_fmt[origin_point..]) catch unreachable;
            }

            /// Writes formated Log to `writer` based on `Options.fmt`
            pub fn fmt(arg: *const LogType, writer: std.io.AnyWriter, timezone: *const zeit.TimeZone) void {
                comptime var origin_point: usize = 0;

                inline while (true) {
                    const delimiter_index: usize = comptime std.mem.indexOfScalarPos(u8, Options.fmt, origin_point, '%') orelse
                        break;
                    ...
                    if (comptime delimiter_index > origin_point)
                        writer.writeAll(Options.fmt[origin_point..delimiter_index]) catch unreachable;

                    switch (placeholder) {
                        inline 'l' => if (arg.level != null) levelFmt(arg, writer),
                        inline 'L' => if (arg.level != null) levelFmt(arg, writer) else writer.writeAll("<NO_LEVEL>") catch unreachable,
                        inline 't' => if (arg.instant != null) timeFmt(arg, writer, timezone),
                        inline 'T' => if (arg.instant != null) timeFmt(arg, writer, timezone) else writer.writeAll("<NO_TIME>") catch unreachable,
                        inline 'c' => if (arg.context != null) scopeFmt(arg, writer),
                        inline 'C' => if (arg.context != null) scopeFmt(arg, writer) else writer.writeAll("<NO_SCOPE>") catch unreachable,
                        inline 'm' => if (arg.message != null) messageFmt(arg, writer),
                        inline 'M' => if (arg.message != null) messageFmt(arg, writer) else writer.writeAll("<NO_MESSAGE>") catch unreachable,
                        inline 's' => if (arg.source_location != null) sourceLocationFmt(arg, writer),
                        inline 'S' => if (arg.source_location != null) sourceLocationFmt(arg, writer) else writer.writeAll("<NO_SOURCE>") catch unreachable,
                ...
                if (comptime origin_point < Options.fmt.len)
                    writer.writeAll(Options.fmt[origin_point..]) catch unreachable;
    ...
    pub fn ConsoleLogProcessor(comptime Options: LogFmtOptions, comptime LogType: type) type {
        return struct {
            ...
            ///
            /// `timezone` is required, NEVER pass null
            pub fn init(timezone: ?*const zeit.TimeZone) LogProcessorType {
                assert(timezone != null);
                return .{
                    .timezone = timezone.?,
                    .out_writer = std.io.getStdOut().writer(),
                };
            }
